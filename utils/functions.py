...
def load_model(load_path):
    ...
    """
    device_ids = [3,4]
    checkpoint = torch.load(load_path,map_location=torch.device('cuda:3'))
    # checkpoint = torch.load(load_path) #5 块GPU的时候，这里好像不用指定，从5块训练切换到3块训练的时候需要上面的语句
...
def init_optimizer(args, model, opt_type="noam"):
    ...
    if opt_type == "noam":
        opt = NoamOpt(dim_input, args.k_lr, warmup, torch.optim.Adam(model.parameters(), betas=(0.9, 0.98), eps=1e-9,lr=0.1), min_lr=args.min_lr)
    elif opt_type == "sgd":
